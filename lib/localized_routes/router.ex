defmodule PhxLocalizedRoutes.Router do
  ...
  @doc """
  Compile hook which creates an alternative Router Helpers module
  ...
  **Pseudo code**
  ...
  defmacro __using__(_options) do
    quote location: :keep do
      @after_compile {PhxLocalizedRoutes.Router, :after_routes_callback}
    end
  end
  ...
  def after_routes_callback(env, _bytecode) do
    ...
    shortest_helper_paths =
      ...
      |> Enum.reduce([], &[Private.shortest_helper(elem(&1, 1)) | &2])
      |> Enum.uniq()
      |> Enum.map(&"#{&1}_path")
    ...
    functions =
      Private.wrapped_or_delegated_functions(
        original_helper_mod,
        loc_helper_mod,
        shortest_helper_paths
      )
